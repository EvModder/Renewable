...
import EvLib.ReflectionUtils.RefConstructor;
...
public class NBTTagUtils{// version = X1.0+
	...
	static final RefMethod methodHasTag = cItemStack.getMethod("hasTag");
	...
	static final RefMethod methodTagHasKey = cNBTTagCompound.getMethod("hasKey", String.class);
	...
	public static String getTagString(ItemStack item, String key){
		...
		Object nmsItem = methodAsNMSCopy.of(null).call(item);
		if(methodHasTag.of(nmsItem).call().equals(false)) return null;
		Object tag = methodGetTag.of(nmsItem).call();
		if(tag == null || methodTagHasKey.of(tag).call(key).equals(false)) return null;
		return (String) methodTagGetString.of(tag).call(key);
	}
